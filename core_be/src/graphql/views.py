...
unhandled_errors_logger = logging.getLogger(
    "src.graphql.errors.unhandled")
handled_errors_logger = logging.getLogger("src.graphql.errors.handled")
...
class GraphQLView(View):
    ...
    def _handle_query(self, request: HttpRequest) -> JsonResponse:
        ...
        if isinstance(data, list):
            ...
            status_code = max(
                (code for response, code in responses), default=200)
        else:
    ...
    def handle_query(self, request: HttpRequest) -> JsonResponse:
        with opentracing.global_tracer().start_active_span("http") as scope:
            ...
            for ip in request_ips.split(","):
                ...
                span.set_tag("http.client_ip_originated_from",
                             settings.REAL_IP_ENVIRON)
                break
