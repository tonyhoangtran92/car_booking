...
class BaseMutation(graphene.Mutation):
    errors = graphene.List(
        ...
        description="List of errors that occurred executing the mutation.",
        deprecation_reason=(
            "Use typed errors with error codes. This field will be removed."
        ),
        required=True,
    )
    ...
    @classmethod
    def check_secret_key(cls, info, secret_key):
        if secret_key != settings.SECRET_KEY_API:
            raise ValidationError(
                {
                    "secret_key": ValidationError(
                        "Invalid_credentials! To get 'secret_key', please contact Papagroup: contact@papagroup.net",
                        code=CoreErrorCode.INVALID,
                    )
                }
            )
...
class BaseByPassTokenMutation(BaseMutation):
    class Meta:
        abstract = True

    @classmethod
    def check_secret_key(cls, info, **data):
